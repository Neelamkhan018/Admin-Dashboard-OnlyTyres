...
const AccessoriesModel = () => {
  ...
  const [accessoriesModels, setAccessoriesModels] = useState([]);
  ...
  const fetchAccessoriesModels = async () => {
    if (!state || !state.brandid) {
      console.error('Brand ID is not available');
      return; // Exit the function if brandid is not available
    }
    try {
      const response = await fetch(`${url.nodeapipath}/get-accessoriesmodel?brandid=${state.brandid}`);
      if (!response.ok) throw new Error('Network response was not ok');
      const data = await response.json();
      setAccessoriesModels(data);
    } catch (error) {
      console.error('Error fetching accessories models:', error);
    }
  };

  useEffect(() => {
    fetchAccessoriesModels();
  }, [state]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${url.nodeapipath}/add-accessoriesmodel`, {
        method: 'POST',
        body: form,
      });
  ...
  const handleToggleActive = async (id, newStatus) => {
    try {
      const response = await fetch(`${url.nodeapipath}/active-accessoriesmodel/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        setAccessoriesModels((prevModel) =>
          prevModel.map((model) =>
  ...
  const deleteAccessoriesModel = async (id) => {
    try {
      const response = await fetch(`${url.nodeapipath}/delete-accessoriesmodel/${id}`, { method: 'DELETE' });
      if (!response.ok) throw new Error('Network response was not ok');
      fetchAccessoriesModels();
    } catch (error) {
      console.error('Error deleting accessories model:', error);
    }
  };
          ...
          <div className="ec-content-wrapper">
            <div className="content">
              <div className="breadcrumb-wrapper breadcrumb-wrapper-2 breadcrumb-contacts">
                <h1>Accessories Model Category</h1>
                <p className="breadcrumbs">
                  <span><a href="#">Home</a></span>
                  <span><i className="mdi mdi-chevron-right"></i></span>Accessories Model Category
                </p>
              </div>
              <div className="row">
                {/* Form to Add an Accessories Model */}
                ...
                {/* Table to Display Accessories Models */}
                  ...
                  <img key={idx} src={`https://tyres.blr1.digitaloceanspaces.com/${item}`} alt={item} className="cat-thumb" />
                                ...
                                <td>
                                  <div className="btn-group">
                                    <Link
                                      to={`/Accessoriesmodeledit/${model._id}`}
                                    ...
                                    <button
                                      ...
                                      onClick={() => deleteAccessoriesModel(model._id)}
                                    >
                                      Delete
                                    </button>
                                  </div>
