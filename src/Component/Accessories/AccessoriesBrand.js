...
export default function AccessoriesBrand() {
  ...
  useEffect(() => {
    const fetchAccessoriesBrandsWithCounts = async () => {
      try {
        const response = await axios.get(`${url.nodeapipath}/get-accessoriesbrands-with-model-counts`);
        setAccessoriesBrands(response.data);
      } catch (error) {
        console.error('Error fetching accessories brands with model counts:', error);
      }
    };

    fetchAccessoriesBrandsWithCounts();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`${url.nodeapipath}/update-accessoriesbrand/${id}`, form, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      } else {
        await axios.post(`${url.nodeapipath}/add-accessoriesbrand`, form, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }
      // Fetch accessories brands again after the operation (add/update)
      const response = await axios.get(`${url.nodeapipath}/get-accessoriesbrand`);
      setAccessoriesBrands(response.data);
      resetForm();
    } catch (error) {
      console.error(`Error ${editMode ? 'updating' : 'adding'} accessories brand:`, error);
    }
  };
  ...
  const handleToggleActive = async (id, newStatus) => {
    try {
      const response = await fetch(`${url.nodeapipath}/active-accessoriesbrand/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        setAccessoriesBrands((prevBrands) =>
  ...
  const deleteAccessoriesBrand = async (id) => {
    try {
      await axios.delete(`${url.nodeapipath}/delete-accessoriesbrand/${id}`);
      // Update the UI without re-fetching the entire list
      setAccessoriesBrands(prevBrands => prevBrands.filter(brand => brand._id !== id));
    } catch (error) {
          ...
          <div className="ec-content-wrapper">
            <div className="content">
              <div className="breadcrumb-wrapper breadcrumb-wrapper-2 breadcrumb-contacts">
                <h1>Accessories Brand Category</h1>
                <p className="breadcrumbs">
                  <span><a href="#">Home</a></span>
                  <span><i className="mdi mdi-chevron-right"></i></span>Accessories Brand Category
                </p>
              </div>
              <div className="row">
                {/* Form to Add/Edit an Accessories Brand */}
                <div className="col-xl-4 col-lg-12">
                  <div className="ec-cat-list card card-default mb-24px">
                    <div className="card-body">
                      <div className="ec-cat-form ec-vendor-uploads">
                        <h4>{editMode ? 'Edit' : 'Add'} Accessories Brand</h4>
                ...
                {/* Table to Display Accessories Brands */}
                  ...
                  <img key={idx} src={`https://tyres.blr1.digitaloceanspaces.com/${item}`} alt={item} className="cat-thumb" />
                                ...
                                <td>
                                  <div className="btn-group">
                                    <Link 
                                      to={`/Accessoriesbrandedit/${brand._id}`} 
                                    ...
                                    <button
                                      onClick={() => navigate("/Accessoriesmodel", { state: { brandid: brand._id } })}
                                      className="btn btn-outline-success"
                                    >
                                      Add Model
                                    </button>
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => deleteAccessoriesBrand(brand._id)}
