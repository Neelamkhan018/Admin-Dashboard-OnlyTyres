...
const AccessoriesModelEdit = () => {
  ...
  useEffect(() => {
    const fetchAccessoriesModelDetails = async () => {
      try {
        const response = await axios.get(`${url.nodeapipath}/get-accessoriesmodel/${id}`);
        ...
        setError('Error fetching accessories model details. Please try again later.');
      } finally {
        setLoading(false);
      }
    };

    fetchAccessoriesModelDetails();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${url.nodeapipath}/update-accessoriesmodel/${id}`, formDataToSend, {
        headers: {
      ...
      if (response.status === 200) {
        navigate('/Accessoriesmodel', { state: { brandid: state.brandid } }); // Redirect to the accessories model list page
      } else {
        setError('Failed to update accessories model');
      }
    } catch (error) {
      console.error('Error updating accessories model:', error);
      setError('Error updating accessories model. Please try again later.');
    }
  };
...
export default AccessoriesModelEdit;