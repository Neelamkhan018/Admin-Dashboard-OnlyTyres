...
const AlloyWheelModelEdit = () => {
  ...
  useEffect(() => {
    const fetchAlloyWheelModelDetails = async () => {
      try {
        const response = await axios.get(`${url.nodeapipath}/get-alloywheelmodel/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${url.nodeapipath}/update-alloywheelmodel/${id}`, formDataToSend, {
        headers: {
      ...
      if (response.status === 200) {
        navigate('/alloywheelmodel', { state: { brandid: state.brandid } }); // Redirect to the alloy wheel model list page
      } else {
        setError('Failed to update alloy wheel model');
      }
    } catch (error) {
      console.error('Error updating alloy wheel model:', error);
      setError('Error updating alloy wheel model. Please try again later.');
    }
  };
...
export default AlloyWheelModelEdit;