...
export default function AlloyWheelBrand() {
  ...
  useEffect(() => {
    ...
    const fetchAlloyWheelBrands = async () => {
      try {
        ...
        setAlloyWheelBrands(response.data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAlloyWheelBrandsWithCounts = async () => {
      try {
        const response = await axios.get(`${url.nodeapipath}/get-alloybrands-with-model-counts`);
        setAlloyWheelBrands(response.data);
      } catch (error) {
        console.error('Error fetching alloy wheel brands with model counts:', error);
      }
    };

    fetchAlloyWheelBrandsWithCounts();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`${url.nodeapipath}/alloy-update/${id}`, form, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      } else {
        await axios.post(`${url.nodeapipath}/add-alloybrand`, form, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }
      // Fetch alloy wheel brands again after the operation (add/update)
      const response = await axios.get(`${url.nodeapipath}/get-alloybrand`);
      setAlloyWheelBrands(response.data);
      resetForm();
    } catch (error) {
      console.error(`Error ${editMode ? 'updating' : 'adding'} alloy wheel brand:`, error);
    }
  };
  ...
  const handleToggleActive = async (id, newStatus) => {
    try {
      const response = await fetch(`${url.nodeapipath}/active-alloybrand/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        setAlloyWheelBrands((prevBrands) =>
  ...
  const deleteAlloyWheelBrand = async (id) => {
    try {
      await axios.delete(`${url.nodeapipath}/alloy-delete/${id}`);
      // Update the UI without re-fetching the entire list
      setAlloyWheelBrands(prevBrands => prevBrands.filter(brand => brand._id !== id));
    } catch (error) {
          ...
          <div className="ec-content-wrapper">
            <div className="content">
              <div className="breadcrumb-wrapper breadcrumb-wrapper-2 breadcrumb-contacts">
                <h1>Alloy Wheel Brand Category</h1>
                <p className="breadcrumbs">
                  <span><a href="#">Home</a></span>
                  <span><i className="mdi mdi-chevron-right"></i></span>Alloy Wheel Brand Category
                </p>
              </div>
              <div className="row">
                {/* Form to Add/Edit an Alloy Wheel Brand */}
                <div className="col-xl-4 col-lg-12">
                  <div className="ec-cat-list card card-default mb-24px">
                    <div className="card-body">
                      <div className="ec-cat-form ec-vendor-uploads">
                        <h4>{editMode ? 'Edit' : 'Add'} Alloy Wheel Brand</h4>
                ...
                {/* Table to Display Alloy Wheel Brands */}
                  ...
                  <img key={idx} src={`https://tyres.blr1.digitaloceanspaces.com/${item}`} alt={item} className="cat-thumb" />
                                ...
                                <td>
                                  <div className="btn-group">
                                    <Link 
                                      to={`/AlloyWheelbrandedit/${brand._id}`} 
                                    ...
                                    <button
                                      onClick={() => navigate("/AlloyWheelmodel", { state: { brandid: brand._id } })}
                                      className="btn btn-outline-success"
                                    >
                                      Add Model
                                    </button>
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => deleteAlloyWheelBrand(brand._id)}
