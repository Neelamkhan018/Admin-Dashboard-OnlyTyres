...
export default function TruckEdit() {
  ...
  // Fetch truck brand details for editing
  useEffect(() => {
    const fetchTruckBrandDetails = async () => {
      try {
        const response = await fetch(`${url.nodeapipath}/get-Truckbrand/${id}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${url.nodeapipath}/Truck-update/${id}`, {
        method: 'PUT',
        body: formDataToSend,
      });
          ...
          <div className="ec-content-wrapper">
            <div className="content">
              <div className="breadcrumb-wrapper breadcrumb-wrapper-2 breadcrumb-contacts">
                <h1>Edit Truck Brand Category</h1>
                <p className="breadcrumbs">
                  <span><a href="#">Home</a></span>
                  <span><i className="mdi mdi-chevron-right"></i></span>Edit Truck Brand Category
                </p>
              </div>
              <div className="row">
